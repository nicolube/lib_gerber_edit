...
pub struct GerberLayerData {
    ...
    /// Leading comments and header commands not reconstructed by [`GerberLayerData::to_commands`]
    /// (file attributes, deprecated image commands), preserved from the source file.
    pub header: Vec<Command>,
...
impl GerberLayerData {
    ...
    pub fn new(ty: LayerType, data: GerberDoc) -> Result<Self, ParseError> {
        ...
        for cmd in all_commands {
            // These are regenerated by `to_commands` from `unit`/`coordinate_format`/
            // `apertures`/`macros`, so keeping the parsed originals would duplicate them
            // in the output; drop them here instead.
            let is_regenerated = matches!(
                cmd,
            ...
            // Not regenerated by `to_commands`, so keep the originals to avoid losing them.
            let is_header_only = matches!(
                cmd,
                Command::ExtendedCode(
            ...
            let is_leading_comment = !past_leading_comments
                && matches!(
                    cmd,
                    Command::FunctionCode(FunctionCode::GCode(GCode::Comment(_)))
                );
            if is_header_only || is_leading_comment {
                header.push(cmd);
            } else {
